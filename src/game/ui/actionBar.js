...
export function renderActionBarHtml({slots, slotKeybinds, getAmmo, getExtra, getCpu, getAmmoCount, missileState}){
  return slots.map((id,index)=>{
    ...
    return `<div class="action-slot ammo-slot ${extra ? "extra-slot" : ""} ${cpu ? "cpu-slot" : ""} ${(ammo || extra || cpu) ? "" : "empty"}" data-action-index="${index}" draggable="${(ammo || extra || cpu) ? "true" : "false"}"><span class="key">${keyCodeToLabel(slotKeybinds?.[index])}</span><div class="cooldown" data-action-cooldown="${index}" style="height:0%"></div>${content}</div>`;
  }).join("");
}

export function updateActionBarDom({root=document, activeLaserSlot, repairBotActive, missileState, getAmmo, getExtra, getCpu, getRepairState, getAmmoCooldown, getEffectiveAmmoCooldown, getAmmoCount}){
  ...
  slots.forEach((el,index)=>{
    ...
    const isActive = activeLaserSlot === index || Boolean(extra?.effect?.repairBot && repairBotActive) || isCpuReady;
    el.classList.toggle("active", isActive);
    el.classList.toggle("empty", !ammo && !extra && !cpu);
