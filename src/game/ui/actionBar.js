...
export function renderActionBarHtml({slots, slotKeybinds, getAmmo, getExtra, getCpu, getFormation, getAmmoCount, missileState, getSlotState}){
  return slots.map((id,index)=>{
    ...
    const rickyHeal = state.kind === "rickyHeal";
    const count = ammo ? getAmmoCount(ammo.id) : 0;
    const filled = Boolean(ammo || extra || cpu || rocketLauncher || formation || rickyHeal);
        ...
        : cpu
          ...
          : rocketLauncher
            ...
            : formation
              ...
              : rickyHeal
                ? `<img src="assets/icons/medical_cross.svg" alt="Balise de soin Ricky"><span class="slot-name">BALISE</span>`
    ...
    return `<div class="action-slot ammo-slot ${extra ? "extra-slot" : ""} ${cpu ? "cpu-slot" : ""} ${rocketLauncher ? "rocket-launcher-slot" : ""} ${formation ? "formation-slot" : ""} ${rickyHeal ? "ricky-heal-slot" : ""} ${filled ? "" : "empty"} ${unavailable ? "unavailable blocked" : ""}" data-action-index="${index}" draggable="${filled && !rickyHeal ? "true" : "false"}" title="${escapeAttr(title)}"><span class="key">${keyCodeToLabel(slotKeybinds?.[index])}</span><div class="cooldown" data-action-cooldown="${index}" style="height:0%"></div>${content}</div>`;
  }).join("");
}
...
export function updateActionBarDom({root=document, activeLaserSlot, selectedRocketAmmo, repairBotActive, missileState, getAmmo, getExtra, getCpu, getFormation, activeDroneFormation, getRepairState, getAmmoCooldown, getEffectiveAmmoCooldown, getAmmoCount, getSlotState}){
  ...
  slots.forEach((el,index)=>{
    ...
    const isActive = available && (activeLaserSlot === index || isSelectedRocket || isSelectedMissile || Boolean(extra?.effect?.repairBot && repairBotActive) || isCpuReady || isActiveFormation || Boolean(rickyHeal && state.usable));
    const filled = Boolean(ammo || extra || cpu || rocketLauncher || formation || rickyHeal);
    ...
    el.classList.toggle("ricky-heal-slot", !!rickyHeal);
    el.classList.toggle("unavailable", filled && !available);
    el.classList.toggle("ready", available && (!!repairState?.ok || isCpuReady || Boolean(rickyHeal && state.usable)));
    ...
    else if(cd && rickyHeal) cd.style.height = `${Math.min(100, Math.max(0, Number(state.cooldown || 0) / Math.max(1, Number(state.cooldownTotal || 60)) * 100))}%`;
    ...
    else if(slotName && rickyHeal) slotName.textContent = state.cooldown > 0 ? `${Math.ceil(state.cooldown)}S` : "BALISE";
  });
}
